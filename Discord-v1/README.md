...
| :--- | :--- | :--- |
| **Authentication** | Who are you? | `middleware/auth.js` — no session ⇒ `401` |
| **Authorisation** | What may you do? | `middleware/permissions.js` — Viewer / DJ / Moderator / Admin |
| **Guild isolation** | Which server may you touch? | `middleware/guildAccess.js` — membership checked per request |
...
All four live in `middleware/guildAccess.js` and are composed by
...
- **CSRF** — origin/referer check on every unsafe method (`middleware/csrf.js`).
...
- **Rate limits** — global (400 req/min/IP) plus per-user buckets on expensive
  operations (`middleware/rateLimit.js`):
...
- **Atomic economy writes** — `utils/db_lock.js` serialises read-modify-write
  sequences per key, preventing points duplication under concurrency.
- **Sanitised errors** — `middleware/errors.js` classifies every failure. Clients
...
Runtime state lives in `json.sqlite` (quick.db / better-sqlite3) and logs in
`logs/`. Both are gitignored. Back up `json.sqlite` — it holds all guild
...
```
Discord-v1/
  index.js                    # bot entry: client, player, command loader
  scheduler.js                # interval job runner
  lib/
    logger.js                 # file logger with rotation + injection guard
  config.json                 # colours, emojis, profanity list

  commands/                   # 100 slash commands, one file each
  events/                     # gateway + player event handlers
  utils/
    db_wrapper.js             # quick.db handle
    db_lock.js                # per-key serialisation for atomic updates
    tickets.js  verification.js  reaction_roles.js  giveaway_service.js
    rankCard.js  welcomeCard.js  (canvas rendering)

  dashboard/
    server.js                 # Express 5 API, binds 0.0.0.0:3000
    socket.js                 # Socket.IO, session-authenticated, per-guild rooms
    middleware/
      auth.js                 # fail-closed session gate
      guildAccess.js          # the four-layer stack, shared by every router
      permissions.js          # Viewer/DJ/Moderator/Admin resolution
      csrf.js                 # origin check on unsafe methods
      rateLimit.js            # per-user, per-operation buckets
      errors.js               # error classification + correlation ids
      devauth.js              # /api/dev token gate (timing-safe)
    routes/                   # guilds, music, auth, dev, stats, permissions
    client/                   # React 19 + Vite source
    public/                   # built SPA (generated)

  scripts/
    keep-tunnel.sh            # Cloudflare tunnel supervisor
    lint-gate.js              # lint ratchet used by CI
  tests/
    security/                 # 9 suites, 137 assertions
    unit/                     # command loader
    manual/                   # smoke tools needing a live bot
  docs/
    engineering-lessons.md    # root causes + how each class is prevented
    audit/                    # audit reports and the reorganisation plan
