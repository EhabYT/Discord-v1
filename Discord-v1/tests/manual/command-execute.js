...
const helpers = require(path.join(root, 'utils/helpers'));
...
(async () => {
    ...
    await t('help-select', async () => {
        const { handleHelpSelect } = require(path.join(root, 'utils/help_interactions'));
    ...
    const files = fs.readdirSync(path.join(root, 'commands')).filter(f => f.endsWith('.js'));
    const defaults = {
        fun: { sub: 'joke' },
        tools: { sub: 'uuid' },
        games: { sub: 'riddle' },
