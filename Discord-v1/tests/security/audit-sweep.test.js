...
function discoverRoutes() {
    ...
    for (const m of srv.matchAll(/app\.use\('(\/api[^']*)',\s*(\w+)\)/g)) {
        ...
        if (!file) continue;
        const src = fs.readFileSync(path.join(root, 'dashboard', 'routes', file), 'utf8');
...
(async () => {
    ...
    check('route discovery found the full API surface', routes.length >= 140, `${routes.length} routes`);
