...
function systemRole(req, botClient) {
    ...
    // Owners retain break-glass access regardless of MFA status. Every other
    // system identity (including read-only SUPPORT) requires the account-level
    // second factor in addition to Discord identity.
    if (base === SYSTEM_ROLES.SUPER_ADMIN) return base;
    if (base !== SYSTEM_ROLES.NONE && req.session?.account?.mfaEnabled !== true) {
        return SYSTEM_ROLES.NONE;
    }
    if (base === SYSTEM_ROLES.SUPPORT) return base;
    // Local development token bootstrap remains available below and is never
    // accepted remotely in production.
