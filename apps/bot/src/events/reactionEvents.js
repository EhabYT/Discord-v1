...
async function handleReactionRole(reaction, user, client, adding) {
    ...
    const stats = await db.get(statsKey) || { messages: 0, voiceTime: 0, reactions: 0 };
    stats.reactions = Math.max(0, (stats.reactions || 0) + (adding ? 1 : -1));
    await db.set(statsKey, stats);

    const { normalize, applyMapping, findForReaction } = require('eb-bot-shared/services/reaction-roles');
