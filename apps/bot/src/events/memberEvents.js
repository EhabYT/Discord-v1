...
function emitLog(guildId, event) {
    try { require('eb-bot-api/websocket/socket').emitLog(guildId, event); } catch (e) {}
}
...
module.exports = [
    {
        ...
        async execute(member, client) {
            ...
            try { require('eb-bot-shared/services/analytics').trackJoin(member.guild.id); } catch(e) {}
            try {
            ...
            if (welcomeCfg && welcomeCfg.enabled && welcomeCfg.channelId) {
                ...
                if (welcomeCh) {
                    ...
                    if (welcomeCfg.cardEnabled) {
                        try {
                            const { generateWelcomeCard } = require('eb-bot-shared/utils/welcome-card');
            ...
            try {
                await require('eb-bot-shared/services/verification').handleJoin(member, db);
            } catch (vErr) {
    ...
    },
    {
        ...
        async execute(member, client) {
            ...
            try {
                const { clearPending } = require('eb-bot-shared/services/verification');
