  ...
  return (
    <div className="page-shell-sm animate-fade-in">
      ...
      <form onSubmit={changePassword} className="cyber-card p-5 space-y-4">
        ...
        <label className="flex items-center gap-2 min-h-[32px] text-xs text-zinc-400 cursor-pointer select-none"><input type="checkbox" checked={passwordForm.revokeOtherSessions} onChange={event => setPasswordForm({ ...passwordForm, revokeOtherSessions: event.target.checked })} className="w-4 h-4 accent-cyan-500 flex-shrink-0" /> Sign out all other sessions</label>
      ...
      <section className="cyber-card p-5 space-y-4">
        <div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-white flex items-center gap-2"><Monitor size={16} className="text-cyan-300" aria-hidden="true" /> Active sessions</p><p className="text-xs text-zinc-400 mt-1">30-minute idle timeout · 24-hour absolute lifetime</p></div><button onClick={() => api.get('/api/account/sessions').then(setSessions)} className="cyber-icon-button" aria-label="Refresh sessions"><RefreshCw size={14} aria-hidden="true" /></button></div>
        <div className="space-y-2">{sessions.length ? sessions.map(item => <div key={item.id} className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-3 flex items-start gap-3"><Monitor size={15} className="text-zinc-400 mt-0.5 flex-shrink-0" aria-hidden="true" /><div className="min-w-0 flex-1"><p className="text-xs text-zinc-200 truncate">{item.device}</p><p className="text-[10px] text-zinc-500 mt-1">Last active {new Date(item.lastActiveAt).toLocaleString()} {item.current && '· Current session'}</p></div><button onClick={() => revokeSession(item.id)} aria-label={`Revoke session ${item.device || ''}`} className="min-h-[32px] px-2 text-[11px] font-medium text-red-300 hover:text-red-200 hover:bg-red-500/10 rounded-lg transition-colors flex-shrink-0">Revoke</button></div>) : <p className="text-xs text-zinc-500">No session metadata available yet.</p>}</div>
      ...
      <section className="cyber-card p-5 space-y-3">
        <p className="font-semibold text-white flex items-center gap-2"><History size={16} className="text-cyan-300" aria-hidden="true" /> Security activity</p>
        <div className="space-y-2 max-h-72 overflow-y-auto">{activity.length ? activity.map((item, index) => <div key={`${item.at}-${index}`} className="flex items-center justify-between gap-3 border-b border-white/[0.05] py-2"><span className="text-xs text-zinc-300">{String(item.event).replaceAll('_', ' ')}</span><time className="text-[10px] text-zinc-500 tabular-nums">{new Date(item.at).toLocaleString()}</time></div>) : <p className="text-xs text-zinc-500">No security events recorded.</p>}</div>
      </section>

      <form onSubmit={performReauth} className="cyber-card p-5 space-y-4">
        <p className="font-semibold text-white">Recent reauthentication</p><p className="text-xs text-zinc-400">Confirm identity for ten minutes before Danger Zone actions.</p>
      ...
      <section className="cyber-card border-red-500/20 p-5 space-y-4"><div><p className="font-semibold text-red-300 flex items-center gap-2"><UserX size={16} aria-hidden="true" /> Danger Zone</p><p className="text-xs text-zinc-400 mt-1">Deactivation signs out every session and retains security/moderation records according to policy.</p></div><input disabled={!reauthenticated} placeholder="Type DELETE" aria-label="Type DELETE to confirm account deactivation" value={reauth.confirmation} onChange={event => setReauth({ ...reauth, confirmation: event.target.value })} className="cyber-input" /><button onClick={deactivate} disabled={!reauthenticated || reauth.confirmation !== 'DELETE'} className="cyber-button-danger">Deactivate account</button></section>
    </div>
  );
}
