...
const { getCached, setCached } = require('../utils/db_wrapper');
...
module.exports = {
    ...
    async execute(message, client) {
        ...
        try { require('../utils/analytics').trackMessage(message.guild.id); } catch(e) {}
        const db = client.db;
        ...
        if (xpEnabled) {
            ...
            if (!ignoredChannels.includes(message.channel.id)) {
                ...
                xpCooldowns.set(xpCooldownKey, Date.now());
