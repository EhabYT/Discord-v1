...
function emitLog(guildId, event) {
    try { require('../dashboard/socket').emitLog(guildId, event); } catch (e) {}
}
...
module.exports = [
    {
        ...
        async execute(member, client) {
            ...
            try { require('../utils/analytics').trackJoin(member.guild.id); } catch(e) {}

            // ── Anti-Raid Detection ──────────────────────────────────────────
            const raidCfg = await client.db.get(`antiraid_${member.guild.id}`) || {};
            if (raidCfg.enabled !== false) {
                const now = Date.now();
                const times = (raidTracker.get(member.guild.id) || []).filter(t => now - t < RAID_WINDOW_MS);
                times.push(now);
                raidTracker.set(member.guild.id, times);
                if (times.length >= RAID_THRESHOLD) {
                    raidTracker.set(member.guild.id, []); // reset burst
                    logger.warn(`[ANTI-RAID] Raid detected in ${member.guild.name} — ${times.length} joins in ${RAID_WINDOW_MS}ms`);
                    emitLog(member.guild.id, {
                        type: 'raid_detected', category: 'security', icon: '🚨',
                        title: 'Raid Detected',
                        description: `${times.length} members joined within ${RAID_WINDOW_MS / 1000}s — possible raid`,
            ...
            emitLog(member.guild.id, {
                ...
                title: 'Member Joined',
                description: `${member.user.tag} joined the server (${member.guild.memberCount} members)`,
                author: { id: member.user.id, tag: member.user.tag, avatar: member.user.displayAvatarURL?.({ size: 32 }) },
                guildId: member.guild.id
            });
            ...
            if (welcomeCfg && welcomeCfg.dmEnabled && welcomeCfg.dmMessage) {
                try {
                    ...
                    logger.warn(`DM welcome failed for ${member.user.tag} (DMs likely disabled)`);
                }
            }
        ...
        async execute(member, client) {
            ...
            if (welcomeCfg && welcomeCfg.leaveEnabled && leaveChId) {
                ...
                if (leaveCh) {
                    ...
                    msg = msg.replace(/{user}/g, member.user.tag).replace(/{guild}/g, member.guild.name).replace(/{count}/g, member.guild.memberCount);
