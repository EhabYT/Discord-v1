...
// Default node options applied when creating a queue node via play commands
client._defaultNodeOptions = {
  skipFFmpeg: true,
  volume: 50,
  leaveOnEmpty: true,
  leaveOnEnd: false,
...
(async () => {
  ...
  if (process.env.UPGRADE_MODE === 'true') {
    logger.info('🚀 Upgrade Mode Enabled: High-performance event handlers active.');
    // Example: Increase event listener max for complex setups
