...
import { Shield, Zap, Lock, Activity, RefreshCw, AlertTriangle, Link2Off, Users, Eye } from 'lucide-react';
...
export default function Security({ guild, guildData }) {
  ...
  useEffect(() => {
    ...
    if (guild?.id) api.get(`/api/guild/${guild.id}/verification`).then(setVerification).catch(() => {});
    loadPerf();
  ...
  const saveVerification = async () => {
    ...
    try {
      await api.post(`/api/guild/${guild.id}/verification`, verification);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-4">
        {/* Member Verification */}
        <SectionCard icon={Lock} title="Member Verification" iconColor="text-cyan-400">
          <div className="space-y-4">
            <CyanToggle
              enabled={verification.enabled}
              onChange={v => setVerification(ver => ({ ...ver, enabled: v }))}
              label="Enable Verification"
              description="Require members to verify before accessing channels"
            />
            <div>
              <label className="text-xs text-gray-500 block mb-1.5">Verified Role</label>
              <select
                value={verification.roleId || ''}
                onChange={e => setVerification(v => ({ ...v, roleId: e.target.value || null }))}
            ...
            <div>
              <label className="text-xs text-gray-500 block mb-1.5">Verification Channel</label>
              <select
                value={verification.logChannelId || ''}
                onChange={e => setVerification(v => ({ ...v, logChannelId: e.target.value || null }))}
                className="cyber-select"
              >
                <option value="">— None —</option>
            ...
            >
              {saving === 'verification' ? 'Saving…' : 'Save Verification'}
            </button>
          </div>
        </SectionCard>
