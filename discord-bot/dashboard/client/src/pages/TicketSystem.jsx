    ...
    <div className="p-6 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="flex border-b border-white/[0.07] mb-5">
        {[
          ...
          { id: 'tickets', label: 'Active Tickets', icon: Ticket, badge: openCount || undefined },
        ].map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={tab === t.id ? 'cyber-tab-active flex items-center gap-1.5' : 'cyber-tab flex items-center gap-1.5'}
          >
            <t.icon size={13} />
            {t.label}
            {t.badge && (
              <span className="cyber-badge-green ml-0.5">{t.badge}</span>
            )}
          </button>
        ))}
      </div>
