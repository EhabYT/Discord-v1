    ...
    async execute(interaction, client, db) {
        ...
        if (sub === 'set') {
            ...
            if (day > daysInMonth(month)) {
                return interaction.reply({ content: `❌ ${MONTHS[month - 1]} only has ${daysInMonth(month)} days.`, ephemeral: true });
            }
        ...
        // ── REMOVE ────────────────────────────────────────────────────────────
        if (sub === 'remove') {
            await db.delete(`birthday_${guildId}_${interaction.user.id}`).catch(() =>
                db.set(`birthday_${guildId}_${interaction.user.id}`, null)
            );
            return interaction.reply({
