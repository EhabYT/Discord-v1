...
const externalSecrets = [
    ...
    'OWNER_ID', 'DEVELOPER_IDS', 'SUPPORT_IDS', 'DEV_TOKEN', 'EMAIL_FROM', 'RESEND_API_KEY', 'SUPABASE_URL', 'SUPABASE_SERVICE_ROLE_KEY', 'ACCOUNT_ENCRYPTION_KEY',
];
...
requireCondition(!workflow.includes('dashboard/') && !workflow.includes('--prefix dashboard'),
    'CI must not reference the legacy dashboard/ path (use apps/web or the root workspace lockfile)');
...
for (const command of ['npm run validate:release', 'npm run lint:gate', 'npm test',
    'npm run audit:prod', 'npm run build:web']) {
