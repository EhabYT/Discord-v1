...
for (const file of files) {
    try {
        const cmd = load(path.join('commands', file));
...
for (const file of fs.readdirSync(path.join(evDir, 'player')).filter(f => f.endsWith('.js'))) {
    try {
        const ev = load(path.join('events/player', file));
...
for (const file of fs.readdirSync(path.join(root, 'utils')).filter(f => f.endsWith('.js'))) {
    try { load(path.join('utils', file)); }
    catch (e) { errors.push(['utils/' + file, e.message]); }
}
...
const dashFiles = [
    ...
    'dashboard/routes/guilds.js',
    'dashboard/routes/music.js',
    'dashboard/routes/stats.js',
    'dashboard/routes/permissions.js',
    'dashboard/middleware/permissions.js',
    'scheduler.js',
    'utils_logger.js',
];
...
const helpers = load('utils/helpers.js');
...
const snipe = load('utils/snipe.js');
