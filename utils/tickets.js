...
async function handleTicketCreate(i, client, db) {
    ...
    if (parentId && typeof parentId === 'string' && !/^\d+$/.test(parentId.trim())) {
        require('../utils_logger').warn(`Invalid ticket category ID "${parentId}" — falling back to no category`);
        parentId = null;
    }
    ...
    try {
        ...
        require('../utils_logger').error('Failed to create ticket channel', { error: err.message });
