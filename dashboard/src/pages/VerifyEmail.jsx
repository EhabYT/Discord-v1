...
export default function VerifyEmail() {
  ...
  return (
    <AuthLayout eyebrow={t('auth.verifyEyebrow', 'Email verification')} title={t('auth.verifyTitle', 'Verify your email')} description={t('auth.verifyDesc', 'Verification links are time-limited, single-use, and replaced when a new link is requested.')} footer={<a href="/profile" className="text-cyan-300 hover:text-cyan-200">{t('auth.verifyFooter', 'Go to profile')}</a>}>
      {state === 'working' && <div className="cyber-info text-sm" role="status">{t('auth.checking', 'Checking verification link…')}</div>}
      {state === 'done' && <div className="cyber-info text-sm text-cyan-100" role="status">{message}</div>}
      {(state === 'missing' || state === 'error') && <div className="space-y-4"><div className="cyber-warning text-sm" role="alert">{message || t('auth.badLink', 'This verification link is missing or invalid.')}</div>{account && !account.emailVerified && <button onClick={resend} className="cyber-button w-full">{t('auth.resend', 'Request a new verification email')}</button>}</div>}
    </AuthLayout>
  );
}
